...
neu_data = open("history_illness.txt", "r").read().split("\n|||\n")
neu_document = []
for text in neu_data:
    text = re.sub("\[.*?\]", "", text)
    text_tokens = regexp_tokenize(text, r"[a-zA-z]+")
    text_tokens_ns = [word for word in text_tokens if not word in all_stopwords]
    neu_document.append((text_tokens_ns, 'history_illness'))

# Remove pos_document for sanity check
documents = pos_document + neg_document + neu_document
